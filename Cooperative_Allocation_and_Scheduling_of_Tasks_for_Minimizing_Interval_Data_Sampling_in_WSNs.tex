...
\begin{figure*}[!t]
\centering
\subfigure[$k\mathrm{=}5$, $r\mathrm{=}2$, the testbed]{\includegraphics[width=0.27\columnwidth]{Figures/evaluation2_data_loss_rate}\label{evaluation2_data_loss_rate}}
\hspace{1pt}\subfigure[The general network]{\includegraphics[width=0.33\columnwidth]{Figures/evaluation4_scene}\label{evaluation4_scene}}
\hspace{1pt}\subfigure[$k\mathrm{=}5$, $r\mathrm{=}2$, the general network]{\includegraphics[width=0.3\columnwidth]{Figures/evaluation4_data_loss_rate}\label{evaluation4_data_loss_rate}}
\caption{A comparison of data loss rate in the testbed is presented by varying the number of nodes in (a). Besides, A general wireless sensor network is deployed by using $22$ nodes with $k=5$ and $r=2$. The yellow star means the collection node; while the red stars represent other nodes in the network, and some of their enlarged pictures are shown as examples. The data loss rate is compared by adopting different strategies of data sampling in (c).}
\label{evaluation3}
\end{figure*}
 
 \begin{figure*}
\centering
\subfigure[$k\mathrm{=}5$, $r\mathrm{=}2$, TOSSIM]{\includegraphics[width=0.26\columnwidth]{Figures/evaluation3_received_data}\label{evaluation3_received_data}}
\hspace{20pt}\subfigure[$k\mathrm{=}5$, $r\mathrm{=}2$, TOSSIM]{\includegraphics[width=0.26\columnwidth]{Figures/evaluation3_data_loss_rate}\label{evaluation3_loss_rate}}\caption{The number of received packets is compared by varying the number of nodes in (a) and so is the data loss rate in (b) for large scale wireless sensor networks simulated in TOSSIM.}
...
Large amount of  sampled data definitely leads to server delay and congestion  in WSNs, degrading the quality of data transmission as a result. In Fig. \ref{evaluation2_data_loss_rate}, we compare data loss rate of different methods by changing the number of nodes in WSNs. It is obvious that the data loss rate becomes larger when the scale of network increases. But \emph{PRUNE} and \emph{CATS} significantly derive the smaller data loss rate than \emph{RANDOM} does. This verifies the benefits of our solution on the improvement of the quality of data transmission by cutting down the unnecessary sampled data. Additionally, we deploy a more general wireless sensor network which consists of $22$ nodes. As illustrated in Fig. \ref{evaluation4_scene}, the yellow star stands for the collection node, and the red stars are used to represent other nodes. Some enlarged pictures of those red stars are shown as examples. The wireless sensor network is a 5-coverage and 2-redundant network with $k=5$ and $r=2$. The sampling tasks are produced under the settings of Section \ref{section_environment_settings}. Moreover, the output power of each a sensor node is adjusted to be a high level in order to ensure that each an area of the network can be detected by at least 5 nodes. We conduct different experiments by varying the average number of tasks for a sensor node. For each an experiment, all the data can be collected for no more than 30 minutes. As illustrated in Fig. \ref{evaluation4_data_loss_rate}, we evaluate the loss rate of the sampled data when we adopts different strategies of data sampling. The average number of sampling tasks for a node varies from 1800 to 5400. It is obvious that \emph{CATS} and \emph{PRUNE} perform better than \emph{RANDOM} because the loss rate of sampled data caused by \emph{CATS} and \emph{PRUNE} is much less than that caused by \emph{RANDOM}. 

%\begin{table}[b]
%\centering
%\tbl{The loss rate of sampled data when we vary the average number of sampling tasks for the network\label{table_evaluation4}}{
%\begin{tabular}{|c|c|c|c|c|}
%\hline
%    Loss rate &  1800 & 2700 & 3600 & 5400\\ \hline
%    RANDOM& 624(0.653) & 826(0.694) & 908(0.748) & 1210(0.776)\\ \hline
%    PRUNE& 1190(0.339) & 1474(0.454) & 1832(0.491) & 2081(0.615)\\ \hline
%    CATS& 1354(0.248) & 1992(0.262) & 2521(0.3) & 3623(0.329)\\ \hline
%\end{tabular}}
%\end{table}
